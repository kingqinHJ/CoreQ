...
{
    return {
        {"calibrated", m_state.calibrated ? "是" : "否"},
...
{
    if (m_cb.logMessage) {
        m_cb.logMessage("  → 读取设备A基本信息...");
    }
    asyncDelay(350, [this]() {
        m_common.id = "A001";
        m_common.version = "1.0.3";
        completeStep(true, "  ✓ 设备A信息: ID=A001, 固件版本=1.0.3");
    });
}
...
{
    if (m_cb.logMessage) {
        m_cb.logMessage("  → 执行设备A快速校准...");
    }
    asyncDelay(300, [this]() {
        m_state.calibrated = true;
        m_state.calibrationValue = 42;
        completeStep(true, "  ✓ 校准完成, 校准值=42");
    });
}
...
{
    if (m_cb.logMessage) {
        m_cb.logMessage("  → 上传固件到设备A...");
    }
    asyncDelay(500, [this]() {
        completeStep(true, "  ✓ 设备A固件上传完成");
    });
}